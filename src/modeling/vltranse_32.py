...
from modeling.word_features import get_triples_features
from detectron2.checkpoint import DetectionCheckpointer
...
class VLTransE(nn.Module):
    def __init__(self, cfg, pooling_size=(7, 7), training=True):
        ...
        self.visual_hidden_dim = 16 * pooling_size[0] * pooling_size[1]
        self.word_feature_dim = 3072
        self.trans_feature_dim = 256
        self.emb_feature_dim = 32
        ...
        **kwargs,
        ...
        for neg_relationships in negative_examples:
            ...
            if triplet_losses["subj"] is None:
                triplet_losses["subj"] = self.triplet_loss["subj"](
                    fc_features["visual"]["subj"],
                    fc_features["textual"]["subj"],
                    neg_fc_features["visual"]["subj"],
                ) + self.triplet_loss["subj"](
                    fc_features["textual"]["subj"],
                    fc_features["visual"]["subj"],
                    neg_fc_features["textual"]["subj"],
                )
                triplet_losses["pred"] = self.triplet_loss["pred"](
                    fc_features["visual"]["pred"],
                    fc_features["textual"]["pred"],
                    neg_fc_features["visual"]["pred"],
                ) + self.triplet_loss["pred"](
                    fc_features["textual"]["pred"],
                    fc_features["visual"]["pred"],
                    neg_fc_features["textual"]["pred"],
                )
                triplet_losses["obj"] = self.triplet_loss["obj"](
                    fc_features["visual"]["obj"],
                    fc_features["textual"]["obj"],
                    neg_fc_features["visual"]["obj"],
                ) + self.triplet_loss["obj"](
                    fc_features["textual"]["obj"],
                    fc_features["visual"]["obj"],
                    neg_fc_features["textual"]["obj"],
                )

                triplet_losses["transr"] = self.triplet_loss["pred"](
                    fc_features["visual"]["subj"] + fc_features["visual"]["pred"],
                    fc_features["visual"]["obj"],
                    neg_fc_features["visual"]["subj"]
                    + neg_fc_features["visual"]["pred"],
                )
                triplet_losses["transr"] += self.triplet_loss["pred"](
                    fc_features["textual"]["subj"] + fc_features["textual"]["pred"],
                    fc_features["textual"]["obj"],
                    neg_fc_features["textual"]["subj"]
                    + neg_fc_features["textual"]["pred"],
                )
